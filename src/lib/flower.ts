/**
 * The brand flower's outline, as drawn in public/logo/flower.svg, and the
 * two boxes it is drawn in.
 *
 * One copy, because the flower is drawn in more than one place and the ways
 * of drawing it differ: <Flower> uses the artwork's own 250×241 box, and the
 * statement band's spinning flower uses a square box centred on the flower
 * itself (see FLOWER_SPIN_VIEWBOX). If the artwork ever changes, it changes
 * here.
 */
...
/** The artwork's own box: the mark, cropped from the lockup's canvas. */
export const FLOWER_VIEWBOX = "0 0 250 241";

/*
 * Sampled along the path, the flower's centre — the point every petal tip is
 * equally far from — sits at (126, 133.25), 13 units below the artwork box's
 * centre. A flower spun about the box's centre wobbles by that much on every
 * turn, 5% of its size, which at the size the statement band spins it up to
 * is a swing of hundreds of pixels. Spun about its own centre it turns in
 * place. The half-side of 134 is the tip radius (133.2) with a hair of room
 * so anti-aliasing never clips a tip.
 */
const CENTRE = { x: 126, y: 133.25 };
const HALF = 134;

/** A square box centred on the flower, for drawing it to spin in place. */
export const FLOWER_SPIN_VIEWBOX = `${CENTRE.x - HALF} ${CENTRE.y - HALF} ${HALF * 2} ${HALF * 2}`;

/**
 * How far in the notches between the petals reach, as a fraction of the
 * spin box's side: 17.5 units of 268. The flower only covers a point once
 * its notches have passed it, so this is what the zoom's end size is worked
 * out from.
 */
export const FLOWER_NOTCH = 17.5 / (HALF * 2);
