...
/*
 * Mesh-drift shader background, from the 21st.dev Shader Builder.
 *
 * Zero dependencies — one WebGL canvas filling its parent. The shader source
 * below is the generator's output, unchanged; the React wrapper is adapted.
 *
...
export interface ShaderBackgroundProps {
  className?: string;
  /** Up to 8; padded to 8 for the uniform array. First is the ground colour. */
  colors: readonly ShaderColor[];
  /** Negative runs the field backwards. 0 freezes it. */
  timeScale?: number;
}
...
export function ShaderBackground({
  ...
  // Flattened here so a new array literal from the parent doesn't rebuild the
  // GL context on every render.
  const key = colors.map((c) => c.join(",")).join("|");
  const palette = useMemo(() => {
    const flat = new Float32Array(24);
    for (let i = 0; i < 8; i++) {
      const c = colors[Math.min(i, colors.length - 1)] ?? [0, 0, 0];
      flat[i * 3] = c[0];
      flat[i * 3 + 1] = c[1];
      flat[i * 3 + 2] = c[2];
    }
    return { flat, count: Math.min(colors.length, 8) };
  ...
  useEffect(() => {
    ...
    /*
     * Upstream compiled and linked without ever reading COMPILE_STATUS or
     * LINK_STATUS, so a shader failure produced a silently black canvas with
    ...
    // Cursor interaction is off: no pointer listeners are registered at all.
    ...
    // Reduced motion freezes the field rather than hiding it — a still
    // gradient is fine, a drifting one is what the setting objects to.
    const speed = reduce ? 0 : timeScale;
    ...
    const resizeCanvas = () => {
      ...
      // Caps total fragments so a 4K hero doesn't melt the GPU.
    ...
    /*
     * An arrow const rather than upstream's `function render(…)`. A hoisted
     * declaration can in principle run before the `if (!gl) return` above it,
     * so TypeScript discards the null-narrowing inside one and every gl/canvas
     * access becomes an error.
     */
    ...
    return () => {
      ...
      gl.deleteProgram(program);
      /*
       * Deferred so React's StrictMode remount reclaims the same context
       * instead of burning one from the browser's small per-page budget.
       */
